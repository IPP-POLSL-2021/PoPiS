...
def loadView():
    ...
    MpGroupedList, MpsList, MpNamesByClub = MPsStats.groupMpsByClub(term_number)
    ...
    selected_mp_data = next((mp for mp in MpsList if mp['lastFirstName'] == selected_mp), None)
    ...
    if selected_mp_data:
        ...
        try:
            ...
            if votings_response.status_code == 200:
                ...
                proceeding_numbers = sorted(list(set(int(v['proceeding']) for v in votings_data)))
                
                # Select proceeding
                proceeding_number = st.selectbox("Numer posiedzenia", proceeding_numbers)
                ...
                selected_date = st.selectbox(
                    ...
                    proceeding_dates,
                    format_func=lambda x: datetime.datetime.strptime(x, '%Y-%m-%d').strftime('%d-%m-%Y')
                )
                
                if st.button("Pokaż głosowania"):
                    # Get all votes for the proceeding first
                    with st.spinner('Pobieranie danych o głosowaniach...'):
                        proceeding_votes_response = votings.get_proceeding_votings(term_number, proceeding_number)
                        ...
                        if proceeding_votes_response.status_code == 200:
                            ...
                            if voting_response.status_code == 200:
                                ...
                                if voting_details:
                                    st.subheader(f"Głosowania z dnia {datetime.datetime.strptime(selected_date, '%Y-%m-%d').strftime('%d-%m-%Y')}")
                                    
                                    # Create tabs for different views
                                    tab1, tab2 = st.tabs(["Szczegóły głosowań", "Statystyki"])
                                    ...
                                    with tab1:
                                        ...
                                        for i, vote in enumerate(voting_details):
                                            ...
                                            full_voting = next(
                                                (v for v in proceeding_votes if str(v.get('votingNumber')) == str(voting_num)),
                                                {}
                                            )
                                            
                                            with st.expander(f"Głosowanie nr {voting_num} - {vote.get('topic', 'Brak tematu')}"):
                                                st.write(f"**Przedmiot głosowania:** {vote.get('topic', 'Brak')}")
                                                st.write(f"**Oddany głos:** {vote.get('vote', 'Brak')}")
                                                st.write(f"**Czas głosowania:** {vote.get('date', 'Brak')}")
                                                
                                                if full_voting:
                                                    st.write("**Wyniki głosowania:**")
                                                    col1, col2, col3, col4 = st.columns(4)
                                                    with col1:
                                                        st.metric("Za", full_voting.get('yes', 0))
                                                    with col2:
                                                        st.metric("Przeciw", full_voting.get('no', 0))
                                                    with col3:
                                                        st.metric("Wstrzymało się", full_voting.get('abstain', 0))
                                                    with col4:
                                                        st.metric("Nieobecni", full_voting.get('notParticipating', 0))
                                    ...
                                    with tab2:
                                        ...
                                        if total_votes > 0:
                                            ...
                                            for vote in voting_details:
                                                ...
                                                logger.info(f"Vote value: {vote_value}")
                                                
                                                # Map vote values
                                                if vote_value.lower() in ['za']:
                                                    votes_count['Za'] += 1
                                                elif vote_value.lower() in ['przeciw']:
                                                    votes_count['Przeciw'] += 1
                                                elif vote_value.lower() in ['wstrzymał się']:
                                                    votes_count['Wstrzymał się'] += 1
                                                elif vote_value.lower() in ['nieobecny']:
                                                    votes_count['Nieobecny'] += 1
                                            
                                            # Log calculated statistics
                                            logger.info("Calculated vote counts:")
                                            for vote_type, count in votes_count.items():
                                                logger.info(f"{vote_type}: {count}")
                                            
                                            st.subheader("Statystyki głosowań")
                                            
                                            # Display statistics in columns
                                            cols = st.columns(4)
                                            vote_types = ['Za', 'Przeciw', 'Wstrzymał się', 'Nieobecny']
                                            ...
                                            st.info("Brak głosowań w wybranym dniu")
                                else:
                                    st.info("Brak głosowań dla wybranego posła w tym dniu")
                            else:
                                st.error("Nie udało się pobrać danych o głosowaniach posła")
                        else:
                            st.error("Nie udało się pobrać danych o posiedzeniu")
            else:
