...
def get_last_n_committee_sitting_dates(committeeCode: str, numberOfSitting: int, term: int) -> Union[List[str], str]:
    ...
    response = requests.get(f'https://api.sejm.gov.pl/sejm/term{term}/committees/{committeeCode}/sittings')
    ...
    if code is None or code == "łącznie":
        ...
        for committee in API_data:
            for member in committee['members']:
                ...
                peoples[member['lastFirstName']] = peoples.get(member['lastFirstName'], 0) + 1
    else:
    ...
    for party, members in committee.items():
        ...
        for person in members:
            ...
            if filtered_MPs:
                ...
                match searchedInfo:
                    case 'edukacja':
                        educationOfMP = str([mp['educationLevel'] for mp in filtered_MPs])
                    case 'okrąg':
                        educationOfMP = str([mp['districtName'] for mp in filtered_MPs])
                    case 'profesja':
                        educationOfMP = str([mp['profession'] for mp in filtered_MPs if 'profession' in mp])
                
                educationOfMP = educationOfMP.strip("[]'")
                educations[educationOfMP] = educations.get(educationOfMP, 0) + 1
...
def get_committee_member_ages(committee: Dict[str, List[str]], term: int = 10, searchedInfo: str = 'birthDate') -> Tuple[pd.DataFrame, Dict[str, List[float]]]:
    ...
    for party, members in committee.items():
        ...
        for person in members:
            ...
            if filtered_MPs:
                dateOfBirth = str([mp[searchedInfo] for mp in filtered_MPs]).strip("[]'")
                
                ageOfMP = current_time.date() - datetime.strptime(dateOfBirth, "%Y-%m-%d").date()
                ages.append(round(ageOfMP.days/365, 1))
